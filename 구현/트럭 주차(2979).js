...
let prices = input[0].split(" ").map(Number);
...
for (let i = 1; i <= 100; i++) {
  const inNum = ins.filter((ele) => ele == i).length;
  const outNum = outs.filter((ele) => ele == i).length;

  if (inNum) num += inNum;
  if (outNum) num -= outNum;

  if (num > 0) {
    answer += num * prices[num - 1];
  }
}
